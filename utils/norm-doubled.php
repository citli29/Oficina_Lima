...
function add_search_name(array $tables){
	print_r($tables);
	$db = getConnection();
	
	foreach ($tables as $table => $def){

		$_1st = "ALTER TABLE {$table} ADD COLUMN {$def['search_name']} VARCHAR(255) NULL";
		echo $_1st;
		$stmt = $db->query($_1st);

		$_2nd = "SELECT id, {$def['name']} FROM {$table}";
		echo $_2nd;
		$stmt = $db->query($_2nd);
		//fetch-all
		$rows = $stmt->fetchAll();
		foreach($rows as $row){
			$q = "UPDATE {$table} SET {$def['search_name']} = ? WHERE id= ?";
			echo $q;
			$stmt = $db->prepare($q);
			$stmt->execute([normalize($row[$def['name']]),$row['id']]);
		}
	}
